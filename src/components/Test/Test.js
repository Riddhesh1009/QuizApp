...
const reducer = (questions, action) => {
  switch (action.type) {
    ...
    case "MARK_FOR_LATER":
      axios
        .get("http://localhost:3001/markForLater", {
          params: {
    ...
    case "UNMARK_FOR_LATER":
      axios
        .get("http://localhost:3001/unMarkForLater", {
          params: {
            id: action.payload.id,
          },
        })
...
function Test() {
  ...
  const handleNext = (e) => {
    if (currentQuestionIndex === 19) {
      if (answer !== "") {
        axios
          .post("http://localhost:3001/insertAnswer", {
            option: answer,
  ...
  const handleSubmit = async () => {
    ...
    axios.get("http://localhost:3001/submitTest").then((res) => {
      console.log(res);
      history.push("/result");
    });
  };
    ...
    <div className="main-test-container">
      <div className="test-banner align-items-center mb-4 flex-column-reverse flex-lg-row">
        <div className="test-banner-text">
          <span className="test-banner-text-mocktest">MOCK TEST</span>
      ...
      <div className="test-parent-container align-items-center mb-4 flex-column flex-lg-row">
        <div className="test-container">
          <div className="test-container-header">
            ...
            <div style={{ cursor: "pointer" }}>
              {currentQuestion.isMFL ? (
                <BookmarkCheckFill
                  onClick={() => {
                    dispatch({
                      type: "UNMARK_FOR_LATER",
                      payload: {
          ...
          <div className="test-container-options">
            <div>
              <input
                ...
                type="radio"
                checked={answer === "A" || currentQuestion.userAnswer === "A"}
                name="A"
                onChange={handleRadio}
              />
              <label for="answer" className="option-text">
                A. {currentQuestion.A}
              </label>
            </div>
            <div>
              <input
                style={{ cursor: "pointer" }}
                type="radio"
                checked={answer === "B" || currentQuestion.userAnswer === "B"}
                name="B"
                onChange={handleRadio}
              />
              <label for="answer" className="option-text">
            ...
            <div>
              <input
                style={{ cursor: "pointer" }}
                type="radio"
                checked={answer === "C" || currentQuestion.userAnswer === "C"}
                name="C"
                onChange={handleRadio}
              />
              <label for="answer" className="option-text">
            ...
            <div>
              <input
                style={{ cursor: "pointer" }}
                type="radio"
                checked={answer === "D" || currentQuestion.userAnswer === "D"}
                name="D"
                onChange={handleRadio}
              />
              <label for="answer" className="option-text">
          ...
          <div className="test-container-btn-container">
            <Button onClick={handlePrev} variant="previous">
              {" "}
              Previous{" "}
            </Button>
            <Button
        ...
        <div className="test-questions">
          ...
          <div class="test-questions-legend">
            <div className="test-questions-legend-item" id="ans"></div>
            <span>Answered</span>
            <div className="test-questions-legend-item" id="unans"></div>
            <span>Unanswered</span>
            <div className="test-questions-legend-item" id="mfl"></div>
            <span>Marked For Later</span>
          </div>
          ...
          <Button onClick={handleSubmit} className="w-75 text-white mb-2">
            Submit
          </Button>
        </div>
      </div>
